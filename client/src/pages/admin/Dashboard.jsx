...
const TT_STYLE     = { contentStyle:{ background:'#122036', border:'1px solid #1a2d45', borderRadius:8 }, labelStyle:{ color:'#e2e8f0' }, itemStyle:{ color:'#8b9ab0' } };
  ...
  return (
    <Card className={`ei-card-hover ${borderClass}`} style={S.card} styles={{ body:{ padding:'20px 22px' } }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <Text style={{ color:'#4f6272', fontSize:11, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.07em', display:'block', marginBottom:10 }}>{label}</Text>
  ...
  return (
    <div>
      ...
      <div style={{ background:'linear-gradient(135deg, #0a2040 0%, #091628 100%)', border:'1px solid #1a3a60', borderRadius:16, padding:'22px 28px', marginBottom:24 }}>
        <Title level={4} style={{ color:'#e2e8f0', margin:0, fontWeight:800 }}>System Dashboard</Title>
        <Text style={{ color:'#4f6272' }}>Full platform overview · {dayjs().format('MMMM D, YYYY')}</Text>
      </div>

      {/* KPI row */}
      <Row gutter={[14, 14]} style={{ marginBottom:20 }}>
        <Col xs={24} sm={12} xl={4}><StatCard label="Customers" value={summary?.totalCustomers||0} sub="Registered" icon={<TeamOutlined />} color="#3b82f6" borderClass="stat-card-blue" /></Col>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom:16 }}>
        <Col xs={24} lg={16}>
          <Card style={S.card} title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Claims Volume — 6 Months</Text>}>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={claimsTrend} margin={{ top:10, right:20, bottom:5, left:-10 }}>
                ...
                <YAxis tick={{ fill:'#4f6272', fontSize:11 }} />
                <Tooltip {...TT_STYLE} />
                <Legend wrapperStyle={{ color:'#4f6272', fontSize:12 }} />
                <Line type="monotone" dataKey="claims" stroke="#3b82f6" strokeWidth={2} dot={{ fill:'#3b82f6', r:4 }} name="Claims" />
        ...
        <Col xs={24} lg={8}>
          <Card style={S.card} title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Policies by Type</Text>}>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={14}>
          <Card style={S.card} title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Revenue by Product Type</Text>}>
        ...
        <Col xs={24} lg={10}>
          <Card style={S.card} title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Claims Status Distribution</Text>}>
              ...
              {claimsByStatus.map(c => (
                <div key={c.status}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                    ...
                    <Text style={{ color:'#4f6272', fontSize:12 }}>{c.count} ({totalC ? Math.round((c.count/totalC)*100) : 0}%)</Text>
                  </div>
                  <Progress percent={totalC ? Math.round((c.count/totalC)*100) : 0} size={['100%',5]} showInfo={false}
                    strokeColor={STATUS_COLORS[c.status]||'#4f6272'} trailColor="#1a2d45" />
                </div>
              ))}
            </div>
          </Card>
