...
const S = {
  ...
  label: { color:'#4f6272', fontSize:12, display:'block', marginBottom:2 },
};

const STATUS_COLORS = {
  active:'#10b981', pending:'#f59e0b', suspended:'#f97316', cancelled:'#ef4444', expired:'#4f6272', pending_renewal:'#3b82f6',
  submitted:'#3b82f6', acknowledged:'#06b6d4', under_review:'#f59e0b', documentation_requested:'#8b5cf6',
  investigation:'#6366f1', assessment:'#eab308', approved:'#10b981', partially_approved:'#84cc16',
  denied:'#ef4444', settled:'#10b981', closed:'#4f6272',
};
  ...
  return (
    <Card className={`ei-card-hover ${borderClass}`} style={S.card} styles={{ body: { padding: '20px 22px' } }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <Text style={{ color:'#4f6272', fontSize:12, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.06em', display:'block', marginBottom:10 }}>{label}</Text>
          <div style={{ color:'#e2e8f0', fontSize:28, fontWeight:800, lineHeight:1 }}>{value}</div>
          {sub && <div style={{ color:'#4f6272', fontSize:12, marginTop:6 }}>{sub}</div>}
        </div>
...
export default function CustomerDashboard() {
  ...
  return (
    <div>
      {/* Welcome banner */}
      <div style={{
        background:'linear-gradient(135deg, #0d2a50 0%, #0a1e3d 50%, #091628 100%)',
        border:'1px solid #1a3a60', borderRadius:16, padding:'28px 32px', marginBottom:24,
        display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:16,
        position:'relative', overflow:'hidden',
      }}>
        <div style={{ position:'absolute', right:-40, top:-40, width:240, height:240, borderRadius:'50%', background:'radial-gradient(circle, rgba(59,130,246,0.08) 0%, transparent 70%)', pointerEvents:'none' }} />
        <div style={{ position:'relative' }}>
          <Title level={4} style={{ color:'#e2e8f0', margin:0, fontWeight:800 }}>
            Good {dayjs().hour() < 12 ? 'Morning' : dayjs().hour() < 17 ? 'Afternoon' : 'Evening'}, {user?.firstName} 👋
          </Title>
          <Text style={{ color:'#4f6272' }}>{dayjs().format('dddd, MMMM D, YYYY')} · Your insurance is active</Text>
        </div>
        <Space>
          <Button type="primary" onClick={() => navigate('/customer/quotes')} icon={<FileTextOutlined />}
            style={{ background:'#3b82f6', border:'none', fontWeight:600, height:40 }}>
            Get a Quote
          </Button>
          <Button onClick={() => navigate('/customer/claims')} icon={<AlertOutlined />}
            style={{ background:'#122036', border:'1px solid #1a2d45', color:'#e2e8f0', height:40 }}>
            File a Claim
          </Button>
        </Space>
      </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom:24 }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <StatCard label="Total Coverage" value={`$${(totalCoverage/1000).toFixed(0)}K`} sub="Across all policies" icon={<CheckCircleOutlined />} color="#3b82f6" borderClass="stat-card-blue" />
        </Col>
        <Col xs={24} sm={12} lg={6}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={14}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Active Policies</Text>}
            extra={<Button type="link" size="small" style={{ color:'#3b82f6', padding:0 }} onClick={() => navigate('/customer/policies')}>
              View all <ArrowRightOutlined />
            </Button>}>
            {activePolicies.length === 0 ? (
              <Empty description={<span style={{ color:'#4f6272' }}>No active policies</span>} image={Empty.PRESENTED_IMAGE_SIMPLE}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {activePolicies.slice(0,5).map(p => {
                  ...
                  return (
                    <div key={p._id} style={{ background:'#060e1a', borderRadius:10, padding:'14px 16px', border:'1px solid #1a2d45' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                        <div>
                          <Text style={{ color:'#e2e8f0', fontWeight:600 }}>{p.product?.name}</Text>
                          <div style={{ color:'#4f6272', fontSize:12, marginTop:2 }}>{p.policyNumber}</div>
                        </div>
                        <div style={{ textAlign:'right' }}>
                          <div style={{ color:'#3b82f6', fontWeight:700, fontSize:15 }}>${p.premium?.amount}<span style={{ color:'#4f6272', fontSize:11 }}>/mo</span></div>
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                        <Text style={{ color:'#4f6272', fontSize:11 }}>Coverage: ${p.coverageDetails?.coverageAmount?.toLocaleString()}</Text>
                        <Text style={{ color: daysLeft < 30 ? '#f59e0b' : '#4f6272', fontSize:11 }}>
                          <ClockCircleOutlined style={{ marginRight:4 }} />
                          {daysLeft > 0 ? `${daysLeft} days left` : 'Expired'}
                        </Text>
                      </div>
                      <Progress percent={pct} size={['100%', 4]} showInfo={false}
                        strokeColor={daysLeft < 30 ? '#f59e0b' : '#10b981'} trailColor="#1a2d45" />
                    </div>
                  );
                })}
              </div>
        ...
        <Col xs={24} lg={10}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Recent Claims</Text>}
            extra={<Button type="link" size="small" style={{ color:'#3b82f6', padding:0 }} onClick={() => navigate('/customer/claims')}>
              View all <ArrowRightOutlined />
            </Button>}>
            {claims.length === 0 ? (
              <Empty description={<span style={{ color:'#4f6272' }}>No claims filed</span>} image={Empty.PRESENTED_IMAGE_SIMPLE} />
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {claims.slice(0,6).map(c => (
                  <div key={c._id} style={{
                    background:'#060e1a', borderRadius:10, padding:'12px 14px',
                    border:`1px solid ${['submitted','under_review'].includes(c.status) ? '#f59e0b33' : '#1a2d45'}`,
                    ...
                    <div>
                      ...
                      <div style={{ color:'#4f6272', fontSize:11, marginTop:2 }}>{c.claimNumber}</div>
                    </div>
                    <div style={{ textAlign:'right' }}>
