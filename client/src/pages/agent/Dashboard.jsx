...
const PRIORITY_CONFIG = { urgent:{color:'#ef4444',bg:'#ef444418'}, high:{color:'#f97316',bg:'#f9731618'}, medium:{color:'#3b82f6',bg:'#3b82f618'}, low:{color:'#4f6272',bg:'#4f627218'} };
const STATUS_COLORS = { submitted:'#3b82f6', acknowledged:'#06b6d4', under_review:'#f59e0b', documentation_requested:'#8b5cf6', investigation:'#6366f1', assessment:'#eab308', approved:'#10b981', partially_approved:'#84cc16', denied:'#ef4444', settled:'#10b981', closed:'#4f6272' };
    ...
    <div>
      ...
      <div style={{ background:'linear-gradient(135deg, #1a0a3d 0%, #0f0a2d 50%, #091628 100%)', border:'1px solid #2a1a5e', borderRadius:16, padding:'24px 28px', marginBottom:24, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative' }}>
          <Title level={4} style={{ color:'#e2e8f0', margin:0, fontWeight:800 }}>Agent Dashboard</Title>
          <Text style={{ color:'#4f6272' }}>Welcome back, {user?.firstName}. You have {actionNeeded.length} items needing attention.</Text>
        </div>
      </div>

      {/* KPI */}
      <Row gutter={[16, 16]} style={{ marginBottom:24 }}>
        <Col xs={24} sm={12} lg={6}><StatCard label="My Customers" value={summary?.totalCustomers||0} sub="Assigned accounts" icon={<TeamOutlined />} color="#3b82f6" borderClass="stat-card-blue" /></Col>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={9}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            title={
              <Space>
                <Text style={{ color:'#e2e8f0', fontWeight:700 }}>Action Required</Text>
            ...
            extra={<Button type="link" size="small" style={{ color:'#3b82f6', padding:0 }} onClick={() => navigate('/agent/claims')}>Manage <ArrowRightOutlined /></Button>}
          >
            {actionNeeded.length === 0 ? (
              <div style={{ textAlign:'center', padding:'32px 0' }}>
                <CheckCircleOutlined style={{ fontSize:40, color:'#10b981', display:'block', marginBottom:12 }} />
                <Text style={{ color:'#4f6272' }}>You're all caught up!</Text>
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {actionNeeded.map(c => {
                  ...
                  return (
                    <div key={c._id} style={{ background:'#060e1a', borderRadius:10, padding:'12px 14px', border:'1px solid #1a2d45', cursor:'pointer' }}
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                        <div>
                          ...
                          <div style={{ color:'#4f6272', fontSize:11, marginTop:2 }}>
        ...
        <Col xs={24} lg={15}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            ...
            extra={<Button type="link" size="small" style={{ color:'#3b82f6', padding:0 }} onClick={() => navigate('/agent/claims')}>View all <ArrowRightOutlined /></Button>}
          >
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {recentClaims.slice(0,8).map(c => {
                ...
                return (
                  <div key={c._id} style={{
                    background:'#060e1a', borderRadius:10, padding:'12px 16px', border:'1px solid #1a2d45',
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      ...
                      <div>
                        <Text style={{ color:'#e2e8f0', fontWeight:600, fontSize:13 }}>{c.customer?.firstName} {c.customer?.lastName}</Text>
                        <div style={{ color:'#4f6272', fontSize:11 }}>{c.claimNumber} · {c.type?.replace(/_/g,' ')}</div>
                      </div>
                    </div>
                    <Space>
                      <div style={{ textAlign:'right' }}>
                        <div style={{ color:'#f59e0b', fontWeight:700 }}>${c.claimedAmount?.toLocaleString()}</div>
                        <div style={{ color:'#4f6272', fontSize:11 }}>{dayjs(c.createdAt).format('MMM D')}</div>
                      </div>
                      <Tag style={{ fontSize:10, background:pc.bg, color:pc.color, border:`1px solid ${pc.color}33` }}>{c.priority}</Tag>
                      <Tag style={{ fontSize:10, background:`${STATUS_COLORS[c.status]}18`, color:STATUS_COLORS[c.status], border:`1px solid ${STATUS_COLORS[c.status]}33` }}>
                        {c.status?.replace(/_/g,' ')}
              ...
              {recentClaims.length === 0 && <div style={{ textAlign:'center', color:'#4f6272', padding:24 }}>No claims yet</div>}
            </div>
          </Card>
        </Col>
      </Row>
