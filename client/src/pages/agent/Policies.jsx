...
export default function AgentPolicies() {
  ...
  const columns = [
    {
      ...
      render: (_, r) => (
        <div>
          <Text style={{ color:'#3b82f6', fontFamily:'monospace', fontSize:11 }}>{r.policyNumber}</Text>
          <div style={{ color:'#4f6272', fontSize:11, marginTop:2 }}>{dayjs(r.startDate).format('MMM D, YYYY')}</div>
        </div>
      ),
    },
    {
      ...
      render: (_, r) => (
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <Avatar size={30} style={{ background:'#3b82f618', color:'#3b82f6', fontWeight:700, fontSize:11, border:'1px solid #3b82f633', flexShrink:0 }}>
          ...
          <div>
            ...
            <div style={{ color:'#4f6272', fontSize:11 }}>{r.customer?.email}</div>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (_, r) => (
        <div>
          <Text style={{ color:'#e2e8f0', fontSize:13 }}>{r.product?.name}</Text>
          <div><Tag style={{ background:'#1a2d45', color:'#8b9ab0', border:'none', fontSize:10 }}>{r.product?.type?.toUpperCase()}</Tag></div>
        </div>
      ),
    },
    {
      ...
      render: (_, r) => (
        <Text style={{ color:'#3b82f6', fontWeight:700 }}>
          ${r.premium?.amount}<span style={{ color:'#4f6272', fontSize:11 }}>/mo</span>
        </Text>
      ),
    },
    {
      title: 'Coverage', width: 120,
      render: (_, r) => <Text style={{ color:'#e2e8f0', fontWeight:600 }}>${r.coverageDetails?.coverageAmount?.toLocaleString()}</Text>,
    },
    {
    ...
    },
    {
      ...
      render: (_, r) => {
        ...
        return <Text style={{ color: soon ? '#f59e0b' : '#4f6272', fontSize:12 }}>{dayjs(r.endDate).format('MMM D, YYYY')}</Text>;
      },
    },
    {
  ...
  return (
    <div>
      <Title level={4} style={{ color:'#e2e8f0', marginBottom:20 }}>Policies</Title>
      ...
      {/* Detail drawer */}
      <Drawer
        title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Policy — {selected?.policyNumber}</Text>}
      ...
      >
        {selected && (
          <>
            <Descriptions column={1} size="small" labelStyle={{ color:'#4f6272', width:130 }} contentStyle={{ color:'#e2e8f0' }}>
              ...
              <Descriptions.Item label="Type"><Tag style={{ background:'#1a2d45', color:'#8b9ab0', border:'none' }}>{selected.product?.type?.toUpperCase()}</Tag></Descriptions.Item>
            ...
            <div style={{ marginTop:16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                <Text style={{ color:'#4f6272', fontSize:12 }}>Coverage period remaining</Text>
                <Text style={{ color:'#4f6272', fontSize:12 }}>
              ...
              <Progress
                ...
                size={['100%', 6]} showInfo={false} strokeColor="#3b82f6" trailColor="#1a2d45"
              />
            </div>

            {selected.coverageDetails?.selectedOptions?.length > 0 && (
              <>
                <Divider style={{ borderColor:'#1a2d45', margin:'16px 0' }}>Coverage Add-ons</Divider>
                <Space wrap>
                  {selected.coverageDetails.selectedOptions.map(o => (
                    <Tag key={o} style={{ background:'#3b82f618', color:'#8b9ab0', border:'1px solid #1a2d45' }}>{o}</Tag>
                  ))}
                </Space>
              </>
            )}
