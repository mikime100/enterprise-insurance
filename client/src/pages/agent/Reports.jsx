  ...
  return (
    <div>
      <div style={{ background:'linear-gradient(135deg, #0a1e3d 0%, #091628 100%)', border:'1px solid #1a3a60', borderRadius:16, padding:'22px 28px', marginBottom:24 }}>
        <Title level={4} style={{ color:'#e2e8f0', margin:0, fontWeight:800 }}>Reports & Analytics</Title>
        <Text style={{ color:'#4f6272' }}>Your portfolio performance overview</Text>
      </div>

      {/* KPIs */}
      <Row gutter={[14, 14]} style={{ marginBottom:20 }}>
        <Col xs={24} sm={12} lg={6}>
          <StatCard label="My Customers" value={summary?.totalCustomers||0} sub="Assigned accounts" icon={<TeamOutlined />} color="#3b82f6" />
        </Col>
        <Col xs={24} sm={12} lg={6}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom:16 }}>
        ...
        <Col xs={24} lg={14}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Policies by Product Type</Text>}>
        ...
        <Col xs={24} lg={10}>
          <Card style={S.card} styles={{ header:{ borderBottom:'1px solid #1a2d45' } }}
            title={<Text style={{ color:'#e2e8f0', fontWeight:700 }}>Claims by Status</Text>}>
            {claimsByStatus.length === 0 ? (
              <div style={{ textAlign:'center', color:'#4f6272', padding:'60px 0' }}>No claims data</div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={claimsByStatus} dataKey="count" nameKey="status"
                    cx="50%" cy="50%" outerRadius={95} innerRadius={50}>
                    {claimsByStatus.map((e,i) => <Cell key={i} fill={STATUS_COLORS[e.status]||'#4f6272'} />)}
                  </Pie>
                  <Tooltip {...TT} formatter={(v,n,p) => [v, p.payload.status?.replace(/_/g,' ')]} />
                  <Legend wrapperStyle={{ color:'#4f6272', fontSize:11 }} formatter={v=>v?.replace(/_/g,' ')} />
                </PieChart>
              </ResponsiveContainer>
            )}
          </Card>
          ...
          {claimsByStatus.map(c => {
            ...
            return (
              <div key={c.status}>
                ...
                <Progress percent={pct} size={['100%', 6]} showInfo={false} strokeColor={sc} trailColor="#1a2d45" />
              </div>
            );
          })}
          {claimsByStatus.length === 0 && (
            <div style={{ textAlign:'center', color:'#4f6272', padding:'20px 0' }}>No claims data</div>
          )}
        </div>
      </Card>
    </div>
